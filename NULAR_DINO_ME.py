...
from core.losses.simdino import SimDINOLoss
...
def run_training(rank, world_size, args):
    ...
    ## Setup the encoder
    student_encoder = get_encoder(args)
    ...
    optimizer, scheduler = get_opt_and_sched(args, student_encoder, student_heads, nstep_total)

    ## Momentum scheduler
    mom_scheduler = cosine_scheduler(args.momentum_teacher, 1, args.nstep, nbatches)
    
    ## Set up metrics
    metrics = defaultdict(list)
    
    ## Load the checkpoint if one has been given
    ## TODO FIX FOR TEACHER
    start_iteration = 0
    if args.restart:
        if not args.state_file:
            if rank==0: print("Restart requested, but no state file provided, aborting")
            sys.exit()
        start_iteration, metrics = load_checkpoint(student_encoder, student_heads, optimizer, args.state_file)
    ...
    for iteration in range(start_iteration, start_iteration+num_iterations):
        ...
        losses_tensor = {name: torch.tensor(0.0, device=device) for name in student_heads.keys()}       
        ...
        for h in student_heads.values(): h.train()
        ...
        for cat_bcoords, cat_bfeats, this_batch_size in train_loader:
            ...
            ## Add to metrics
            total_enc_align_tensor += alignment(student_encoded_batch)
            ...
            if len(buffer_enc) < nbuffer:
                with torch.no_grad():
                    buffer_enc .append(student_encoded_batch.detach().to("cpu", non_blocking=True))
                    buffer_proj.append(student_proj_batch.detach().to("cpu", non_blocking=True))
        ...
        av_losses = {
            ...
            for name in student_heads.keys()
        }
        ...
        if rank==0:
            ...
            log_grad_norm(student_encoder.module, "encoder", writer, iteration)
            log_grad_rms(student_encoder.module, "encoder", writer, iteration)
            log_grad_over_wgt(student_encoder.module, "encoder", writer, iteration)

            log_grad_norm(student_heads["proj"].module, "proj", writer, iteration)
            log_grad_rms(student_heads["proj"].module, "proj", writer, iteration)
            log_grad_over_wgt(student_heads["proj"].module, "proj", writer, iteration)
        ...
        if rank==0 and iteration%25 == 0 and iteration != 0:
            save_checkpoint(teacher_encoder, teacher_heads, optimizer, args.state_file+".check"+str(iteration), iteration, metrics, args)
    ...
    if rank==0:
        save_checkpoint(teacher_encoder, teacher_heads, optimizer, args.state_file, iteration, metrics, args)
